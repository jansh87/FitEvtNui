...
//systematics
    ...
    vErr.push_back( new t_bf("Dpilnu",2,2,0.8,t_error::_xlnu)); //neu, vorher 4.5%
    vErr.push_back( new t_bf("D*pilnu",2,4,1.,t_error::_xlnu)); //eigentlich 130%
...
void PerfFit(){
    ...
    double **xs;
    double ***errors;
    pseudoPars = new double [nComponents + nErrors];
    ...
    for(gi_Error = 0; gi_Error<(__calcerrors?nErrors:1); gi_Error++)
    {
        ...
        for(int i = 0; i<(__toydata?50:1); i++){
        	if(__toydata){
...
//  belleStyle->SetOptFit(1011); // show probability, parameters and errors
  ...
  belleStyle->SetStatFontSize(0.04);
  belleStyle->SetStatX(0.9);
  belleStyle->SetStatY(0.9);
  belleStyle->SetStatW(0.20);
  belleStyle->SetStatH(0.10);
